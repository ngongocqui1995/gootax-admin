...
const ThemeColor: React.ForwardRefRenderFunction<HTMLDivElement, ThemeColorProps> = ({
  ...
  return (
    <div className={`${baseClassName} ${hashId} !mt-0`}>
      ...
      <ColorPicker onChange={(value) => onChange && onChange(value.toHexString())}>
        <Tooltip key="custom" title={'Tự chọn'}>
          <Tag className={`${baseClassName}-block ${hashId}`} color="gray" />
        </Tooltip>
      </ColorPicker>
    </div>
  );
